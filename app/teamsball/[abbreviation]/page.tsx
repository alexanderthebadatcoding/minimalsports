...
export default function TeamPage() {
  ...
  const apiUrl = abbreviation
    ? `https://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/teams/${abbreviation.toLowerCase()}`
    : null;
      ...
      <div className="max-w-4xl mx-auto p-6">
        {data.team?.logos?.[0]?.href && (
          <div className="flex justify-center mt-6">
            <Image
              src={data.team?.logos[0]?.href || "/na.png"}
              alt={`${data.team?.displayName || "Team"} logo`}
              width={124}
              height={124}
                ...
                {competition?.competitors?.map((team) => (
                  ...
                  >
                    <div className="flex-row items-center">
                      <Image
                        src={team.team?.logos[0]?.href || "/na.png"}
                        alt={`${team.team?.displayName || "Team"} logo`}
                        width={100}
                        height={100}
                      />
                      <div className="text-center">
                        <span className="font-bold text-lg ">
                          {team.team?.shortDisplayName || "TBD"}{" "}
